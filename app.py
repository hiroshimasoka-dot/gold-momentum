...
# RSI reference lines as traces
rsi_ob_line = [rsi_ob] * len(df)
rsi_os_line = [rsi_os] * len(df)
rsi_50_line = [50] * len(df)
fig.add_trace(go.Scatter(x=df.index, y=rsi_ob_line, mode="lines", name="買われ過ぎ", line=dict(color="#FF2D55", width=1, dash="dash"), showlegend=False), row=2, col=1)
fig.add_trace(go.Scatter(x=df.index, y=rsi_os_line, mode="lines", name="売られ過ぎ", line=dict(color="#00E5A0", width=1, dash="dash"), showlegend=False), row=2, col=1)
fig.add_trace(go.Scatter(x=df.index, y=rsi_50_line, mode="lines", name="中線", line=dict(color="#1e2535", width=1, dash="dot"), showlegend=False), row=2, col=1)
...
# MACD zero line as trace
macd_zero = [0] * len(df)
fig.add_trace(go.Scatter(x=df.index, y=macd_zero, mode="lines", line=dict(color="#1e2535", width=1), showlegend=False), row=3, col=1)
